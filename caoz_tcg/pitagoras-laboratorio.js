...
(function(){
  ...
  window.abrirLaboratorioEditor=function(){
    ...
    for(const [tipo,etiqueta] of [['isometrico','CENITAL'],['laseres','ESQUIVA'],['fps','FPS · RETRO'],['carrera','TRES CARRILES'],['orbital','NUEVO · SUPERVIVENCIA ESPACIAL'],['duelo','NUEVO · MEMORIA']]){
      ...
      b.onclick=async()=>{
        if(e.ocupado||e.cerrado)return;e.ocupado=true;lista.inert=true;salir.disabled=true;
        ...
        finally{e.ocupado=false;if(!e.cerrado){lista.inert=false;salir.disabled=false;b.focus({preventScroll:true});}}
      };
    }
    const pie=nodo('footer','elPie',''),estado=nodo('span','','20 segundos · 3 vidas',pie);estado.setAttribute('aria-live','polite');
    const salir=nodo('button','elSalir','Menú principal',pie);salir.type='button';salir.onclick=()=>d.close();
  ...
  addEventListener('load',()=>{if(new URLSearchParams(location.search).get('editor')==='1')abrirLaboratorioEditor();},{once:true});
})();
