...
entran al Domo con un Protagonista como Líder y un mazo de 40 cartas. Sus archivos no
...
| `carta-pintor.js`, `carta-diseno.js`, `carta-diseno.css`, `fuentes/` | Cartas de Colección pintadas en canvas (color, relieve, metal, máscara holo): clásico en Normal/Foil, full art en Dorada. En la Colección con el relieve horneado; datos de `CARDS`, ilustración por el proveedor de arte. Tipografías OFL en `fuentes/`. |
| `sobres-escena.js`, `sobres-apertura.js/css`, `sobres-revelacion.js` | El sobre en WebGL con material pintado (relieve, metal, holo, portada) y su apertura. Con WebGL y pintor, el bonche sale como cartas del visor 3D (`visor-3d-gl.js`, una ranura por carta); si no, cartas planas. La apertura sólo dibuja: fases e inventario siguen en quien la monta. |
| `fx-aliento.js` | Efectos de fuego y ácido en WebGL + Canvas 2D (`entrada`, `ataque`, `quemar`). En la partida, sólo `quemar` desde `fxDeath` cuando muere Machete; el resto se revisa en la sección `fuego`. Si no puede dibujar, devuelve `false` y sigue la animación de siempre. |
| `cortinilla.js`, `cortinilla-juego.js` | La cortinilla del arranque con las cartas 3D (`visor-3d-gl.js` + `carta-pintor.js`) y su montaje en el juego: pantalla de carga, espera de la cuenta (`caoz:cuenta-lista`), doce cartas de la colección con su acabado, una vez por sesión (`sessionStorage`). No corre con `?test`; `CAOZ_CORTINILLA_JUEGO.forzar()` la reproduce. Se revisa en la sección `cortinilla`. |
| `tema-domo.css` | El lenguaje del visor en el resto del juego: Cinzel/Cormorant, paneles de cristal con filo de oro, botones, burbujas de combate, avisos, registro. Sólo estilo, cargado al final por `final.js`; sus selectores llevan `:root` para ganar al pulido AAA. |
| `carta-juego.js`, `carta-juego.css` | Las cartas de la partida (mano, mesa, revelaciones, ficha `.big`) con la cara del pintor sin cifras, desde `ilustrar()`. La carta del juego sigue entera por dentro y transparente; coste/ATQ/VIDA quedan vivos sobre sus gemas. Caras en caché por carta, edición, arte y encuadre; estado en `data-piel` (el render reescribe `className`). También el dorso con el logo. |
| `visor-3d.js`, `visor-3d.css`, `visor-3d-gl.js` | El detalle de Colección es la carta en 3D (montada con `CAOZ_VISOR3D.montar`, también a pantalla completa). Con WebGL propio (sin dependencias) dibuja la carta con luz, relieve, metal, laca y holo; sin WebGL, capas CSS sin `preserve-3d`. Sólo dibuja. |
| `cuenta-modelo.js`, `cuenta-ui.js`, `cuenta.css`, `cuenta-juego.js/css` | Acceso por correo y perfil desde Extras. El montaje espera la identidad y mantiene el diálogo fuera del lienzo escalado. La demostración aislada no se distribuye con el juego. |
| `cuenta-acceso.js` | Coordinador compartido por juego y laboratorio: acceso obligatorio, vínculo confirmado, sincronización y estados de conexión. No abrir partidas ni resolver invitaciones antes de completar el acceso. |
| `cuenta-progreso.js`, `cuenta-servicio.js` | Captura completa, recibo local de identidad sin tokens, respaldo, conflictos y cola persistente por cuenta/entorno. Reapertura offline sólo con cuenta ya verificada y vinculada. No sumar inventarios ni repetir premios al restaurar; no reanuda un combate a mitad de turno. |
