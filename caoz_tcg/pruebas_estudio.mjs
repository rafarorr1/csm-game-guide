...
const catalogoProduccionInicial=await dato('/api/arte/catalogo'),anterior=JSON.stringify(await dato('/api/sfx/catalogo')),arteAnterior=JSON.stringify(catalogoProduccionInicial);assert.deepEqual(catalogoProduccionInicial.titulos,[]);
...
assert.equal((await req('/api/estudio/arte/titulo/tal','PATCH',JSON.stringify({titulo:'Sin sesión'}),{'If-Match':'0',Cookie:''})).status,401);
assert.equal((await req('/api/estudio/arte/titulo/tal','PATCH',JSON.stringify({titulo:'Otro origen'}),{'If-Match':'0',Origin:'https://mal.invalid'})).status,403);
assert.equal((await req('/api/estudio/arte/titulo/tal','PATCH',JSON.stringify({titulo:'Sin revisión'}))).status,428);
r=await req('/api/estudio/arte/titulo/tal','PATCH',JSON.stringify({titulo:'Nombre del borrador'}),{'If-Match':'0'});assert.equal(r.status,200);assert.equal(r.headers.get('cache-control'),'no-store');const tituloBorrador=await r.json();assert.equal(tituloBorrador.nombre.id,'tal');assert.equal(tituloBorrador.nombre.titulo,'Nombre del borrador');assert.equal(tituloBorrador.nombre.revision,1);
const borrador=await dato('/api/estudio/arte/privado');assert.equal(borrador.cartas[0].hash,sha(arte));assert.equal(borrador.titulos.find(t=>t.id==='tal').titulo,'Nombre del borrador');
...
const catalogoBetaVacio=await dato('/api/arte/catalogo','GET',undefined,{},eb);assert.equal(catalogoBetaVacio.cartas.length,0,'Guardar no altera beta');assert.deepEqual(catalogoBetaVacio.titulos,[],'El borrador tampoco filtra títulos al catálogo público');
...
let catalogoProduccion=await dato('/api/arte/catalogo');assert.equal(catalogoProduccion.cartas[0].acabado,'dorado');assert.deepEqual(catalogoProduccion.titulos,[{id:'tal',revision:1,titulo:'Nombre del borrador'}]);
// Un reset es un tombstone en el borrador: CAS invalida la huella previa y debe llegar a ambos destinos.
const estadoAntesResetTitulo=await dato('/api/estudio/arte/estado');
r=await req('/api/estudio/arte/titulo/tal','PATCH',JSON.stringify({titulo:null}),{'If-Match':'1'});assert.equal(r.status,200);assert.equal((await r.json()).nombre.revision,2);
assert.equal((await req('/api/estudio/arte/publicar/beta','POST',undefined,{'If-Match':estadoAntesResetTitulo.beta.huella})).status,409);
const estadoTituloNulo=await dato('/api/estudio/arte/estado');assert.equal(estadoTituloNulo.beta.pendientes,1);assert.equal(estadoTituloNulo.produccion.pendientes,1);
await dato('/api/estudio/arte/publicar/beta','POST',undefined,{'If-Match':estadoTituloNulo.beta.huella});
assert.deepEqual((await dato('/api/arte/catalogo','GET',undefined,{},eb)).titulos,[],'Beta no publica el tombstone');
assert.deepEqual((await dato('/api/arte/catalogo')).titulos,[{id:'tal',revision:1,titulo:'Nombre del borrador'}],'Producción queda intacta hasta su publicación propia');
const borradorNulo=await dato('/api/estudio/arte/privado'),tombstoneBorrador=borradorNulo.titulos.find(t=>t.id==='tal');assert.equal(tombstoneBorrador.titulo,null);assert.equal(tombstoneBorrador.revision,2);
await dato('/api/estudio/arte/publicar/produccion','POST',undefined,{'If-Match':estadoTituloNulo.produccion.huella});
catalogoProduccion=await dato('/api/arte/catalogo');assert.deepEqual(catalogoProduccion.titulos,[],'Producción tampoco expone tombstones');
const privadoProduccion=await dato('/api/arte/privado'),tombstoneProduccion=privadoProduccion.titulos.find(t=>t.id==='tal');assert.equal(tombstoneProduccion.titulo,null);assert.equal(tombstoneProduccion.revision,2);assert.equal(typeof tombstoneProduccion.actualizado,'string');
assert.equal((await req('/api/estudio/arte/titulo/tal','PATCH','[]',{'If-Match':'2'})).status,400,'El proxy conserva la respuesta de cliente inválida del endpoint de títulos');
