...
node --check "$AQUI/fx-aliento.js" || exit 1
for f in fx-ascension.js fx-poderes.js campo-lugar.js efectos-claude.js; do node --check "$AQUI/$f" || exit 1; done
...
for f in coleccion-modelo.js coleccion-juego.js coleccion-ui.js sobres-escena.js sobres-apertura.js sobres-revelacion.js carta-pintor.js carta-diseno.js carta-juego.js visor-3d-gl.js visor-3d.js fx-aliento.js fx-ascension.js fx-poderes.js campo-lugar.js efectos-claude.js cortinilla.js cortinilla-juego.js; do node --check "$AQUI/$f" || exit 1; done
...
cp "$AQUI/fx-aliento.js" "$PAGES/$DESTINO/fx-aliento.js"
...
git add "$DESTINO/fx-aliento.js" "$DESTINO/fx-ascension.js" "$DESTINO/fx-poderes.js" "$DESTINO/campo-lugar.js" "$DESTINO/efectos-claude.js" "$DESTINO/cortinilla.js" "$DESTINO/cortinilla-juego.js"
...
comprobar_cloudflare(){
  ...
  for j in $(seq 1 12); do
    ...
    if [ "$ok" = "1" ]; then
      local esp_fx srv_fx
      esp_fx="$(shasum -a 256 "$AQUI/fx-aliento.js" | cut -d" " -f1)"
      if [ "$DESTINO" = "tcg" ]; then
        srv_fx="$(curl_portal "$CF_URL$prefijo/fx-aliento.js?cb=$marca" | shasum -a 256 | cut -d" " -f1)" || ok=0
      else
        srv_fx="$(curl -sL "$CF_URL/fx-aliento.js?cb=$marca" | shasum -a 256 | cut -d" " -f1)"
      fi
      [ "$srv_fx" = "$esp_fx" ] || ok=0
    fi
    if [ "$ok" = "1" ]; then
...
for i in $(seq 1 10); do
  ...
  if [ "$CODIGO" = "200" ] && [ "$SERVIDO" = "$ESPERADO" ] && [ "$CODIGO_MOTOR" = "200" ] && [ "$SERVIDO_MOTOR" = "$ESPERADO_MOTOR" ] && [ "$SERVIDO_MOVIL" = "$ESPERADO_MOVIL" ] && [ "$SERVIDO_FINAL" = "$ESPERADO_FINAL" ]; then
    ...
    for f in invitaciones-compartidas.js final-core.js campana-mesa.js campana-personaje.js campana-deseo.js campana-pitagoras.js campana-secreto.js campana-honores.js pitagoras-pruebas.js pitagoras-mundos.js pitagoras-cine.js pitagoras-laboratorio.js pitagoras-fps.js pitagoras-pixel.js pitagoras-combate.js pitagoras-mesa.js dado-fisico.js moneda-fisica.js polish-aaa.js mulligan-ui.js mulligan-ui.css arte-remoto.js arte-vistas.js nombres-cartas.js estudio.js estudio.html coleccion.css coleccion-modelo.js coleccion-juego.js coleccion-ui.js sobres-escena.js sobres-apertura.js sobres-revelacion.js fx-aliento.js fx-ascension.js fx-poderes.js campo-lugar.js efectos-claude.js cortinilla.js cortinilla-juego.js sobres-apertura.css carta-pintor.js carta-diseno.js carta-juego.js visor-3d-gl.js visor-3d.js carta-diseno.css carta-juego.css tema-domo.css visor-3d.css fuentes/cinzel.woff2 fuentes/cormorant-garamond.woff2 fuentes/cormorant-garamond-italica.woff2 cuenta-modelo.js cuenta-progreso.js cuenta-servicio.js cuenta-ui.js cuenta-acceso.js cuenta-juego.js cuenta.css cuenta-juego.css cuenta-servidor.js cuenta-correo.js sw.js manifest.webmanifest portal.html portal.css portal.js pwa-rescate.html pwa-rescate.css pwa-rescate.js fisico/index.html fisico/fisico.css fisico/Caoz-PnP-Duelo-del-Pergamino.zip art/pitagoras-abismo-v216.webp art/esbirro-editor-v219.webp art/moneda-cara-v245.webp art/moneda-cruz-v245.webp; do
    ...
    curl -fsSL "https://rafarorr1.github.io/csm-game-guide/$DESTINO/fx-aliento.js?cb=$(date +%s)" -o /tmp/caoz-verificar-fx-aliento.js || exit 1
    cmp -s "$AQUI/fx-aliento.js" /tmp/caoz-verificar-fx-aliento.js || { rojo "fx-aliento.js no coincide con la versión local"; exit 1; }
