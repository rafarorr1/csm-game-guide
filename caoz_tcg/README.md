...
La sincronización indica los guardados pendientes y conserva una cola para
reintentar cuando regrese la conexión. No incluye reanudar un combate a mitad
de turno. Los deseos escritos en el final de campaña siguen siendo locales.

Se guarda el correo verificado, nombre e identificador del jugador, además del
...
Desde la build 233, `estudio.html` (también `/estudio`) es un panel privado por
internet con el mismo diseño y acceso del estudio de sonidos. La beta está en
...
sesión guardó primero, el panel pide revisar esa versión antes de sobrescribirla.

El juego recibe las ilustraciones y encuadres en escritorio y móvil sin reiniciar
la partida. Conserva originales y arte descargado cuando falta conexión.
**Recuperar borradores locales** permite revisar lo guardado por el editor
anterior en ese navegador; no lo elimina ni lo publica automáticamente.

El catálogo se deriva de las cartas y mazos del motor. La vista previa del
panel tiene su propio CSS: si cambia el marco o la distribución del juego,
revisar también `estudio.css`. No representa estados de combate ni animaciones.
Para crear arte puede usarse la proporción 5:7, con 750×1050 px como referencia.
El encuadre se aplica al archivo completo, sin cortar el original del repositorio.

El backend requiere Cloudflare Pages y la configuración existente de sonidos.
Un servidor estático permite abrir el juego con arte original, pero no guardar
cambios remotos desde el panel. Arquitectura, seguridad y validación en
[ILUSTRACIONES.md](ILUSTRACIONES.md).
