...
(function(global){
  ...
  // Identidades estables del juego. No se consulta el DOM, las imágenes ni el
  // orden de CARDS para barajar; las mismas semillas producen las mismas manos.
  ...
  function nuevaMemoria(s){
    s.rondaMemoria++;const n=Math.min(7,4+s.rondaMemoria),simbolos=[...MEMORIA_CARTAS];
    ...
    s.elegidasMemoria=[];s.faseMemoria='mostrar';s.limiteMemoria=Math.max(4,5-(s.rondaMemoria-1)*.4);s.reveladoMemoria=Math.max(1.3,1.5-(s.rondaMemoria-1)*.05);s.hastaMemoria=s.t+s.reveladoMemoria;s.resultadoMemoria='';s.elegirAnterior=null;
  }
  ...
  function dibujarLaseres(s,c){
    ...
    for(const r of m.rayos){
      if(r.edad<0)continue; // Un disparo programado aún no tiene aviso ni emisor.
    ...
    for(const r of m.rayos){if(r.edad<0)continue;const aviso=r.edad<r.aviso,z=limite(r.edad/r.aviso,0,1),fin=extremosRayo(r);for(const e of fin){if(!e)continue;const p=P(e.x,e.y),ang=Math.atan2(cy-p.y,cx-p.x);c.save();c.translate(p.x,p.y);c.rotate(ang);const rr=esc*.29;pol(c,[[-rr,-rr],[rr*.6,-rr],[rr,-rr*.5],[rr,rr*.5],[rr*.6,rr],[-rr,rr]],'#1f2d37','#9da69a');linea(c,-rr*.8,-rr*.7,rr*.5,-rr*.7,'#5a767c',1);linea(c,-rr*.8,rr*.7,rr*.5,rr*.7,'#5a767c',1);halo(c,rr*.6,0,esc*(.6+z*.6),aviso?'#f7b25c44':'#fb61467d');elipse(c,rr*.75,0,rr*.2,rr*.52,aviso?'#e8bb74':'#ffe2b3');c.strokeStyle=aviso?'#f5cf93':'#ffe3c9';c.lineWidth=Math.max(1,esc*.055);c.beginPath();c.arc(0,0,rr*1.45,-Math.PI/2,-Math.PI/2+TAU*z);c.stroke();c.restore();}}
  ...
  function instalarMemoriaCSS(){
    if(document.getElementById('ppMemoriaCSS'))return;const estilo=document.createElement('style');estilo.id='ppMemoriaCSS';estilo.textContent=`
      ...
      .pitPrueba .ppMemoriaCarta{appearance:none;position:relative;display:block;flex:0 0 var(--pm-ancho,90px);width:var(--pm-ancho,90px);height:var(--pm-alto,132px);min-width:44px;min-height:44px;padding:0;border:1px solid #a997715d;border-radius:3px;background:#121925;color:#e9dec8;box-shadow:0 7px 16px #0007,inset 0 0 0 3px #0b101a,inset 0 0 0 4px #aa956247;cursor:pointer;touch-action:manipulation;outline-offset:3px;transition:translate .15s,box-shadow .15s,border-color .18s;overflow:hidden;isolation:isolate;font-family:system-ui,sans-serif}
  ...
  function pintarRecuerdo(lienzo,id,ilustrada){
    // El arte conserva identidad y encuadre. Sólo este lienzo usa resolución
    // limitada y vecino más próximo; nunca se modifica la ilustración fuente.
    lienzo.width=96;lienzo.height=112;const c=lienzo.getContext('2d'),carta=cartaMemoria(id);c.clearRect(0,0,96,112);if(!carta)return;
    c.imageSmoothingEnabled=false;c.fillStyle='#161524';c.fillRect(0,0,96,112);
  ...
  API.actualizadores.duelo=function(s){
    ...
    if(s.fase==='jugando'&&u.rondaArte!==m.rondaMemoria){u.rondaArte=m.rondaMemoria;u.artes=new Set(m.cartasMemoria.filter(id=>{const img=imagenMemoria(id,true);return img&&img.complete&&img.naturalWidth;}));}
    ...
    const ancho=s.ui.campo.clientWidth,alto=s.ui.campo.clientHeight,tam=ancho+'x'+alto+'x'+n;if(tam!==u.tam&&ancho>0&&alto>0){u.tam=tam;const apaisado=ancho>alto*1.45||ancho>900,columnas=apaisado?n:n===7?4:3,filas=Math.ceil(n/columnas),gap=ancho<360?7:10,disponible=alto-(apaisado?88:155),cw=Math.max(44,Math.min(142,(ancho-28-gap*(columnas-1))/columnas,(disponible-gap*(filas-1))/filas/1.48));u.mesa.style.setProperty('--pm-ancho',cw+'px');u.mesa.style.setProperty('--pm-alto',(cw*1.48)+'px');u.mesa.style.setProperty('--pm-gap',gap+'px');u.mesa.style.setProperty('--pm-nombre',Math.max(8,Math.min(14,cw*.112))+'px');u.mesa.style.setProperty('--pm-cifra',Math.max(14,Math.min(27,cw*.24))+'px');u.mesa.style.setProperty('--pm-numero',Math.max(10,Math.min(18,cw*.16))+'px');u.mesa.style.maxWidth=(cw*columnas+gap*(columnas-1)+1)+'px';}
  };
