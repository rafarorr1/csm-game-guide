...
(function(){
  ...
  const ENCUADRES={tal:{x:45,y:41,z:118},machete:{x:50,y:30,z:100},rey:{x:50,y:30,z:100},bartolomeo:{x:50,y:30,z:100},eric:{x:50,y:30,z:100}};
  const RIVALES=['rey','bartolomeo','eric'],BOTONES=['entrar','golpear','matar','morirMachete','reiniciar'];
  ...
  async function montar(){
    poner($('atacante'),await cara('tal'));poner($('aliado-machete'),await cara('machete'));for(const id of RIVALES)poner($('rival-'+id),await cara(id));
  ...
  async function preparar(){
    ...
    $('entrar').onclick=entrar;$('golpear').onclick=golpear;$('matar').onclick=matar;$('morirMachete').onclick=morirMachete;$('reiniciar').onclick=()=>{if(!ocupado)montar();};
    window.CAOZ_FUEGO_REVISION=Object.freeze({entrar,golpear,matar,morirMachete,montar});
  }
