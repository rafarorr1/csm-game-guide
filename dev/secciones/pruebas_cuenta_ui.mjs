...
async function recorrer(v,base){
 ...
 try{
  await cargar();await foto('inicio');await paso('Inicio centrado y contenido en pantalla',()=>medir('inicio'));
  ...
  await elegirEscenario('vacio');await autenticar();await fase('perfil');await paso('Jugador nuevo entra sin inventar progreso',async()=>{assert.match(await p.locator('.cuentaResumen').innerText(),/Tu aventura/);await medir('nuevo-sin-progreso');});
  await elegirEscenario('entrar');await autenticar();await fase('perfil');await paso('Cuenta existente recupera la nube',async()=>{assert.match(await p.locator('.cuentaResumen').innerText(),/Lyra/);});
  await elegirEscenario('nuevo');await p.locator('[data-foco=invitado]').click();await fase('invitado');await foto('invitado');await paso('Invitado y vuelta a proteger progreso',async()=>{await medir('invitado');assert.match(await p.locator('.cuentaResumen').innerText(),/Fender/);await p.locator('[data-foco=proteger]').click();await fase('inicio');});
  await paso('Sin conexión permite reintentar al recuperarla',async()=>{await p.locator('#cuentaLabAjustes').click();await p.locator('#cuentaLabConexion').check();await p.locator('#cuentaLabAjustes').click();await p.locator('[data-foco=enviar]').click();await p.locator('.cuentaError:not([hidden])').waitFor();assert.match(await p.locator('.cuentaError').innerText(),/conexión/i);await p.locator('#cuentaLabAjustes').click();await p.locator('#cuentaLabConexion').uncheck();await p.locator('#cuentaLabAjustes').click();await codigo();});
...
/* El modo puntual verifica rojo/verde en390px, sin repetir los otros51 casos.
