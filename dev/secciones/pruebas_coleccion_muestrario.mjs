  ...
  for(const [vista,width,height]of [['desktop',1440,900],['movil',390,844],['movil',320,568]]){
    ...
    try{
      for(const acabado of ['normal','foil','dorado']){
        ...
        await pagina.locator('#coleccionPanel[open] .coleccionCopias').first().waitFor();
        ...
        const datos=await pagina.evaluate(acabado=>{
          ...
          return {cantidad:minis.length,total:m.ids().length,
            ...
            cartas:minis.every(n=>{const carta=n.querySelector('.coleccionCarta'),img=carta.querySelector('img'),v=CAOZ_ARTE.version(n.dataset.carta,acabado);return carta.dataset.coleccionAcabado===acabado&&n.querySelector('.coleccionCopias').textContent==='×3'&&(!img||img.getAttribute('src')===v.url);}),
      ...
      assert.deepEqual(errores,[]);console.log('✓ '+vista+' '+width+'×'+height+': tres series completas, selector, enlaces y memoria temporal');
