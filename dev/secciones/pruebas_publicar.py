...
''')
    ...
    def test_sobres_exportador_propio_y_registro_explicito(self):
        self.assertEqual(set(p.SECCIONES), {'coleccion', 'rey', 'sobres', 'cuenta', 'interacciones', 'epilogo-gero', 'pitagoras', 'heroe', 'mulligan', 'invitaciones', 'estudio-nombres', 'portal', 'cartas', 'fuego', 'poderes', 'lugares', 'estudio-campos', 'cortinilla'})
        original_run = subprocess.run
        llamadas = []
        def registrar(args, **opciones):
            if args[0] == 'node':
