/* Aliento de Thal: al entrar, una onda de ácido verde alcanza a la mesa rival;
   al atacar, Thal se envuelve en llamas, escupe un chorro y el objetivo arde.
   La pieza sólo dibuja sobre nodos que le entrega la mesa: no altera daño,
   reglas ni muertes. WebGL pinta el quemado cuando la cara de la carta está
   disponible; Canvas 2D conserva un efecto completo si no lo está. */
'use strict';
(function(){
  const TAU=Math.PI*2;
...
void main(){
  ...
  function ataque(host,o){
    ...
    // Una imagen de arte remoto puede no permitir textura WebGL (o todavía no
    // haber terminado de pintar). En ese caso el Canvas 2D sigue haciendo el
    // aliento y la ceniza: un efecto visual jamás puede detener una partida.
    if(gl&&o.imagenObjetivo){try{gl.textura(o.imagenObjetivo);}catch(e){console.warn('Fuego sin textura:',e.message);gl.destruir();gl=null;}}
    else if(gl){gl.destruir();gl=null;}
    let W=1,H=1,dpr=1;
    ...
    return new Promise(resolve=>{
      ...
      function cuadro(ahora){
        ...
        // El daño se anuncia también cuando el objetivo termina en ceniza: la
        // cifra viene del motor, después de responder Trampas y salvamentos.
        if(!danoPuesto&&tt>=1.2){danoPuesto=true;const r=rel(obj);if(o.dano!=null)mostrarDano(host,r.x+r.w/2,r.y+r.h*.45,'-'+o.dano,vel);}
